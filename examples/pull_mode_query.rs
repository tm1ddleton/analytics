...
use analytics::{
    AnalyticsDag, NodeParams, AssetKey, DateRange, InMemoryDataProvider, TimeSeriesPoint,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Step 5: Statistics
    println!("\n=== Summary ===");
    let valid_points: Vec<_> = results.iter()
        .filter(|p| !p.close_price.is_nan())
        .collect();
    
    if !valid_points.is_empty() {
        let min_vol = valid_points.iter()
            .map(|p| p.close_price)
            .fold(f64::INFINITY, f64::min);
        let max_vol = valid_points.iter()
            .map(|p| p.close_price)
