...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Creating sample data for {} assets (3 months each)...", tickers.len());
    ...
    let result = replay.run(assets.clone(), date_range, move |asset, _timestamp, value| {
        ...
        asset_prices_clone.lock().unwrap()
            .entry(asset)
    ...
    for asset in &assets {
        if let Some(prices) = prices_map.get(asset) {
            ...
            let latest_vol = volatility.last()
    ...
    println!("Expected per asset: ~{} trading days", result.total_points / assets.len());
    println!("All assets replayed in chronological order across {} trading days",
