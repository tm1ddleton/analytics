...
use analytics::{
    AnalyticsDag, NodeParams, AssetKey, DateRange, InMemoryDataProvider, TimeSeriesPoint, NodeId,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (asset, base_price) in &assets {
        ...
        for day in 0..252 {  // 1 trading year
    ...
    for (ticker, (_, node_id)) in &node_map {
        if let Some(data) = results.get(node_id) {
            let valid_points: Vec<_> = data.iter()
                .filter(|p| !p.close_price.is_nan())
                .collect();
            
            if !valid_points.is_empty() {
                let avg_vol = valid_points.iter()
                    .map(|p| p.close_price)
                    .sum::<f64>() / valid_points.len() as f64;
                
                println!("{:<6} - {} data points, avg volatility: {:.6}",
                    ticker, data.len(), avg_vol);
            }
        }
    }
    ...
    println!("Note: Sequential timing would be ~{:?} (3x single-asset time)",
        parallel_elapsed * 3);
