...
use analytics::{
    ...
    DateRange, AssetKey, PushModeEngine, AnalyticsDag, NodeParams,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = replay.run(vec![aapl.clone()], date_range, move |_asset, timestamp, value| {
    ...
    println!("Total log return: {:.6} ({:.2}%)", 
        total_return, (total_return.exp() - 1.0) * 100.0);
