...
fn main() {
    ...
    let (dag, data_node_id, returns_node_id, vol_node_id) =
        builder.build_dag().expect("Failed to build DAG");
    ...
    let prices = vec![
        100.0, 102.0, 101.0, 105.0, 104.0, 107.0, 106.0, 109.0, 108.0, 110.0, 109.0, 112.0, 111.0,
        114.0, 113.0,
    ];
    ...
    // Final statistics
    let valid_vols: Vec<f64> = volatility.iter().filter(|v| !v.is_nan()).copied().collect();
