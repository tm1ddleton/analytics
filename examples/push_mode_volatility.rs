...
use analytics::{
    AnalyticsDag, PushModeEngine, AssetKey, NodeParams,
    NodeOutput,
};
use chrono::Utc;
...
fn main() {
    ...
    engine.register_callback(vol_node, Box::new(move |output| {
        if let NodeOutput::Single(points) = output {
            if let Some(last_point) = points.last() {
                vol_updates_clone.lock().unwrap().push(last_point.close_price);
    ...
    let data_history = engine.get_history(data_node)
    ...
    let vol_history = engine.get_history(vol_node)
