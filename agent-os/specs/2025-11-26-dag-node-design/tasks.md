...
- [x] Define `LagAnalytic` descriptor trait + ensure window layer can supply `lag +1` slices.
...
- [x] Refactor analytic primitives so the registry drives execution via generic trait interfaces (no more `calculate_returns`/`calculate_volatility` helpers); each primitive declares its required inputs (asset, current/lagged prices, window slices) and is registered by `AnalyticType`.
...
- [x] Introduce a LagAnalytic descriptor + window interface so lag nodes declare “I need the last `lag + 1` points”.
- [x] Add calendar metadata derived from the data provider so analytics know which timestamps exist and how much burn-in to request (e.g., 20-day volatility with 5-day returns needs prices from `T - 25`).
- [x] Wire the pull-mode DAG construction to simulate push updates using the calendar sequence, so push/pull share the same execution path for overlapping windows.
