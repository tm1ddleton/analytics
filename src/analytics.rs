...
use crate::time_series::{TimeSeriesPoint, DateRange};
use crate::dag::{Node, NodeId, NodeParams, NodeOutput, DagError};
...
pub fn create_data_provider_node(
    node_id: NodeId,
    asset: AssetKey,
    date_range: DateRange,
) -> Node {
...
pub fn execute_returns_node(
    ...
    let price_data = match &inputs[0] {
        ...
        _ => return Err(DagError::ExecutionError(
...
pub fn execute_volatility_node(
    node: &Node,
    inputs: &[NodeOutput],
) -> Result<NodeOutput, DagError> {
    if inputs.is_empty() {
...
impl VolatilityQueryBuilder {
    ...
    pub fn build_dag(&self) -> Result<(crate::dag::AnalyticsDag, NodeId, NodeId, NodeId), DagError> {
...
mod tests {
    ...
    fn test_returns_with_known_price_sequence() {
        ...
        // ln(110/100) ≈ 0.09531
        assert!((returns[1] - 0.09531).abs() < 0.0001, "Second return incorrect");
        
        // ln(105/110) ≈ -0.04652
        assert!((returns[2] - (-0.04652)).abs() < 0.0001, "Third return incorrect");
        
        // ln(115/105) ≈ 0.09090
        assert!((returns[3] - 0.09090).abs() < 0.0001, "Fourth return incorrect");
    }
    ...
    fn test_returns_increasing_sequence() {
        ...
        assert!(returns[0].is_nan());
        assert!(returns[1] > 0.0, "Increasing price should give positive return");
        assert!(returns[2] > 0.0, "Increasing price should give positive return");
    }
    ...
    fn test_returns_decreasing_sequence() {
        ...
        assert!(returns[0].is_nan());
        assert!(returns[1] < 0.0, "Decreasing price should give negative return");
        assert!(returns[2] < 0.0, "Decreasing price should give negative return");
    }
    ...
    fn test_volatility_edge_case_window_larger_than_data() {
        ...
        assert_eq!(volatility.len(), 2, "Should return volatility for each point");
    }
    ...
    fn test_node_identification_hashing() {
        ...
        assert_ne!(hash1, hash3, "Different assets should produce different hashes");
    }
    ...
    fn test_returns_query_builder() {
        ...
        let data_pos = exec_order.iter().position(|&id| id == data_node_id).unwrap();
        let returns_pos = exec_order.iter().position(|&id| id == returns_node_id).unwrap();
        assert!(data_pos < returns_pos, "Data node should come before returns node");
    }
    ...
    fn test_volatility_query_builder() {
        ...
        let data_pos = exec_order.iter().position(|&id| id == data_node_id).unwrap();
        let returns_pos = exec_order.iter().position(|&id| id == returns_node_id).unwrap();
        let vol_pos = exec_order.iter().position(|&id| id == volatility_node_id).unwrap();
        
        assert!(data_pos < returns_pos, "Data node should come before returns node");
        assert!(returns_pos < vol_pos, "Returns node should come before volatility node");
    }
    ...
    fn test_query_returns_api_exists() {
        ...
        let result = AnalyticsQuery::query_returns(
            &asset,
            &date_range,
    ...
    fn test_query_volatility_api_exists() {
        ...
        // Test that the API exists (will return error without DataProvider)
        let result = AnalyticsQuery::query_volatility(
            &asset,
            10,
            &date_range,
            OutputMode::TimeSeries,
    ...
    fn test_volatility_calculation_with_real_returns() {
        ...
        for (i, &vol) in volatility.iter().enumerate() {
            ...
            assert!(vol >= 0.0, "Volatility at index {} should be non-negative", i);
        }
    }
