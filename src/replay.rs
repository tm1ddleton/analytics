...
impl std::fmt::Display for ReplayResult {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "Replay complete: {} points ({} successful, {} failed)", 
            self.total_points, self.successful, self.failed)?;
        writeln!(f, "Simulated: {} to {}", 
...
impl ReplayEngine {
    ...
    {
        log::info!("Starting replay: {} assets, date range {} to {}",
            assets.len(), date_range.start, date_range.end);
        ...
        for (asset, point) in data {
            ...
            match data_callback(asset.clone(), point.timestamp, point.close_price) {
                ...
                Err(e) => {
                    ...
                    log::warn!("Failed to replay {} at {}: {}",
                        asset, point.timestamp, error_msg);
        ...
        let elapsed = end_time.signed_duration_since(start_time).to_std()
            .unwrap_or(Duration::from_secs(0));

        log::info!("Replay complete: {}/{} successful", successful, total_points);
...
mod tests {
    ...
    fn test_configuration_with_method_chaining() {
        ...
        engine.set_delay(Duration::from_millis(75))
    ...
    fn test_replay_result_display() {
        ...
        let result = ReplayResult {
            ...
            simulated_end: DateTime::from_naive_utc_and_offset(
                NaiveDate::from_ymd_opt(2024, 3, 31).unwrap().and_hms_opt(0, 0, 0).unwrap(),
                Utc,
            ),
        };
    ...
    fn test_load_multiple_assets_merges_correctly() {
        ...
        // Add AAPL data
        provider.add_data(aapl.clone(), vec![
            TimeSeriesPoint::new(
                Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap(),
                100.0,
            ),
        ...
        // Add MSFT data
        provider.add_data(msft.clone(), vec![
            TimeSeriesPoint::new(
                Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap(),
                200.0,
            ),
        ...
        let engine = ReplayEngine::new(Arc::new(provider));
        let result = engine.load_and_sort_data(&[aapl, msft], &date_range).unwrap();
    ...
    fn test_chronological_sorting_with_interleaved_timestamps() {
        ...
        let engine = ReplayEngine::new(Arc::new(provider));
        let result = engine.load_and_sort_data(&[aapl.clone(), msft.clone()], &date_range).unwrap();
    ...
    fn test_run_calls_data_callback_for_each_point() {
        ...
        let result = engine.run(vec![aapl], date_range, |_asset, _timestamp, _value| {
            callback_count += 1;
            Ok(())
        }).unwrap();
    ...
    fn test_run_returns_correct_replay_result_counts() {
        ...
        let result = engine.run(vec![aapl], date_range, |_asset, _timestamp, value| {
            if value > 105.0 {
    ...
    fn test_progress_callback_invoked_for_each_point() {
        ...
        engine.run(vec![aapl], date_range, |_asset, _timestamp, _value| {
            Ok(())
        }).unwrap();
    ...
    fn test_callback_fails_on_specific_asset() {
        ...
        // Fail only for MSFT
        let result = engine.run(vec![aapl.clone(), msft.clone()], date_range, 
    ...
    fn test_callback_fails_intermittently() {
        ...
        // Fail on even days
        let result = engine.run(vec![aapl], date_range, |_asset, timestamp, _value| {
    ...
    fn test_integration_replay_collects_data_correctly() {
        ...
        // Run replay
        let result = replay.run(vec![aapl.clone()], date_range, move |asset, timestamp, value| {
            collected_data_clone.lock().unwrap().push((asset.clone(), timestamp, value));
            Ok(())
        }).unwrap();
    ...
    fn test_multi_asset_replay_chronological_order() {
        ...
        let result = engine.run(
            ...
            move |asset, timestamp, _value| {
                processing_order_clone.lock().unwrap().push((asset.clone(), timestamp.day()));
                Ok(())
            }
        ).unwrap();
    ...
    fn test_replay_performance_benchmark() {
        ...
        for month in 1..=12 {
            ...
            for day in 1..=days_in_month {
                ...
                for asset in &assets {
                    ...
                    let existing = provider.get_time_series(
                        asset,
                        &DateRange::new(
                            NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(),
        ...
        let result = engine.run(assets, date_range, |_asset, _timestamp, _value| {
            Ok(())
        }).unwrap();
        ...
        assert!(actual_ms >= lower_bound && actual_ms <= upper_bound,
