...
use crate::dag::{AnalyticsDag, NodeId, NodeOutput, Node, NodeParams, DagError};
...
impl fmt::Display for PushError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PushError::OutOfOrder { timestamp, last_computed } => {
                write!(f, "Out of order data: timestamp {:?} is before last computed {:?}", 
...
impl fmt::Display for InitError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            InitError::InsufficientHistoricalData { required, available } => {
                write!(f, "Insufficient historical data: required {} days, available {}", 
                       required, available)
            }
...
impl PushModeEngine {
    ...
    pub fn push_data(
        ...
        let exec_order = self.dag.execution_order_immutable()
        ...
        let sorted_affected: Vec<NodeId> = exec_order.into_iter()
...
mod tests {
    ...
    fn test_push_data_engine_not_initialized() {
        ...
        assert!(matches!(result.unwrap_err(), PushError::EngineNotInitialized));
    }
    ...
    fn test_invoke_callbacks() {
        ...
        engine.register_callback(node_id, Box::new(move |_| {
    ...
    fn test_callback_error_doesnt_halt() {
        ...
        engine.register_callback(node_id, Box::new(|_| {
            panic!("Test panic");
        })).unwrap();
    ...
    async fn test_data_provider_to_returns_integration() {
        ...
        assert!(returns_history.is_ok(), "Should be able to get returns history");
        
        let history = returns_history.unwrap();
        assert!(history.len() >= 1, "Returns node should have at least 1 output");
    }
    ...
    async fn test_full_chain_data_returns_volatility() {
        ...
        assert!(returns_history.len() >= 9, "Returns node should have outputs");
        
        let vol_history = engine.get_history(vol_node).unwrap();
        assert!(vol_history.len() >= 5, "Volatility node should have outputs");
    }
    ...
    async fn test_callback_fires_with_real_data() {
        ...
        engine.register_callback(data_node, Box::new(move |output| {
        ...
        assert!(*callback_fired.lock().unwrap(), "Callback should have been invoked");
    }
}
